...
def get_scores(paths: list) -> list:
...
def display_plots(results: list, names_model: list):            
    # Create plots for metrics and loss evolution
    _, axs = plt.subplots(len(names_model), 2, figsize=(12, 10))
...
def get_confusion_matrix(name_model, only_mol):
    ...
    with open(f"../results/colab/{name_model}/eval_data.spacy", "rb") as f:
    ...
    p = tp / (tp + fp) * 100
    r = tp / (tp + fn) * 100
    return confusion_matrix, [p, r]

def display_confusion_matrix(cf_mtx, names_model: list):
    # Display confusion matrix    
    _, axs = plt.subplots(1, len(names_model), figsize=(12, 3))
    for i, name in enumerate(names_model):
        sns.heatmap(cf_mtx[name], annot=True, cmap='Blues', fmt='d', ax=axs[i])
        axs[i].set_xlabel('Annotated')
        axs[i].set_ylabel('Predict')
        axs[i].set_title("Confusion matrix of " +  name, y=-0.25)
